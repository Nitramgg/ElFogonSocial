...
const Muro = () => {
  ...
  const getPosts = async () => {
    try {
      const res = await axios.get('https://elfogonsocial.onrender.com');
      setPosts(res.data);
    } catch (error) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.post('https://elfogonsocial.onrender.com', { text }, config);
      setText('');
      getPosts();
    } catch (error) {
  ...
  const deletePost = async (id) => {
    if (window.confirm('¿Quieres eliminar este mensaje?')) {
      ...
      try {
        ...
        await axios.delete(`https://elfogonsocial.onrender.com${id}`, config);
        setPosts(posts.filter(p => p._id !== id));
      } catch (error) {
        console.error("Error al borrar:", error.response?.data || error.message);
        alert("No tienes permiso para borrar este post o hubo un error de conexión.");
      }
    }
  };
    ...
    <div className="muro-container">
      <header className="header">
        ...
        <div className="user-nav">
          {/* Si no hay foto, mostramos un circulo celeste con la inicial */}
          {user?.foto ? (
            <img src={user.foto} alt="" className="user-avatar-nav" />
          ) : (
          ...
          <div key={post._id} className="post-card">
            <div className="post-header">
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <img src={post.user?.foto || 'https://via.placeholder.com/40'} className="avatar-post" alt="" />
